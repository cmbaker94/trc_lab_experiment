function threshold_combined_crest_length(Tinfo,gammab,IMthresh,minareaIMST,minarea,distconnect,samprate,pltflag,ixlim,iylim,idxdy,ixsel)
...
% distconnect =  6; % smoothing between regions
% IMthresh =  0.2;%1.5; % adjust the mean cross-shore pixel intensity for thresholding imagery
% samprate = 8;  %Hz
% pltflag = 1;
% ixsel = [27 31.5];
% 
...
imthreshmethod = 2;
% display('switch back to using the ''filled'' rectified images')
...
for i = 1:length(imageno)
    ...
    Iob = imagethresh(IM,IMthreshproc,minareaIMST,idxmin,idxmax); % threshold imagery
...
%         h3 = pcolor(ax3,stereo.orig.x,stereo.orig.y,flipud(squeeze(stereo.orig.z(:,:,i))-squeeze(stereo.orig.msse(:,:,i))));
        h3 = pcolor(ax3,stereo.orig.x,stereo.orig.y,flipud(squeeze(stereo.orig.z(:,:,i))-stereo.orig.msse));
...
subname = ['_x',num2str(round(ixsel(1))),'to',num2str(round(ixsel(2))),'_ycrest_res',num2str(idxdy),'_filled'];
...
for ni = 1:n
    ...
    if crestno>0
        ...
        a = (props.MajorAxisLength*idxdy)/2;
        b = (props.MinorAxisLength*idxdy)/2;
        Xc = props.Centroid(1)*idxdy+ixsel(1);
        Yc = props.Centroid(2)*idxdy+min(Y,[],'all');
