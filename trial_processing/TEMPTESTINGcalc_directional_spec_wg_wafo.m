...
%         Spec    = dat2dspec([Time' Z],[pos types,bfs],H,insHz*(256),361,dirmethod);
        [S,f,theta,k] = MLEDirSpec(Y,X,Nfft,dt,dx,a,d)
    elseif smoothz == 1
%         Spec    = dat2dspec([Time' Z],[pos types,bfs],H,(1/tpass)*(256),361,dirmethod);
        ...
        dx = 1./[0.1:0.1:20];
        a = deg2rad(2);
        [S,f,theta,k] = MLEDirSpec(Z,[xd' yd'],Nfft,dt,dx,a);
    end
    
    % Spec.S = Spec.S;
    spec = nanmean(abs(S),3);
    S(:,:,iT) =spec;
    dirs = theta*(180/pi);
    freqs = f;
    ...
    [~,ihigh] = nanmin(abs(freqs-frange(1)));
    [~,ilow] = nanmin(abs(freqs-frange(2)));
    Sf(:,iT)      = nansum(spec,2)*dtheta;
    % Sd(:,iT)      = nansum(Spec,1)*df;
    for ig = 1:size(spec,2)
        Sd(ig,iT) = -trapz(freqs(ilow:ihigh),spec(ilow:ihigh,ig));
    end
...
if loadprev ==  0
    psname = ['E:\data\processed\conditions\','dirspec_cond_comp\dirspec_wg_Hs',num2str(100*Tinfo.Hs),'m_geom',num2str(geom),'_',dirmethod,'_wafo_dir',num2str(diff(dirs(1:2))),'.mat'];
