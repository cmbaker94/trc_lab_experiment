function extract_timeseries_insitu_loc(Tinfo)
...
for j = 1:length(stepx)
    ...
    if length(stepx)>1
        if Tinfo.filt == 1
            fntime = [savefolder,'cam_array_timeseries_stepx',num2str(stepx(j)),'_stepy',num2str(stepy(j)),'_filt.csv'];
            fnxyz = [savefolder,'cam_array_xy_stepx',num2str(stepx(j)),'_stepy',num2str(stepy(j)),'_filt.csv'];
        else
        ...
        if Tinfo.filt == 1
            fntime = [savefolder,'cam_array_timeseries_filt.csv'];
            fnxyz = [savefolder,'cam_array_xy_filt.csv'];
        else
...
for j = 1:length(stepx)
    ...
    if length(stepx)>1
        if Tinfo.filt == 1
            fntime = [savefolder,'lidar_array_timeseries_stepx',num2str(stepx(j)),'_stepy',num2str(stepy(j)),'_filt.csv'];
            fnxyz = [savefolder,'lidar_array_xy_stepx',num2str(stepx(j)),'_stepy',num2str(stepy(j)),'_filt.csv'];
        else
        ...
        if Tinfo.filt == 1
            fntime = [savefolder,'lidar_array_timeseries_filt.csv'];
            fnxyz = [savefolder,'lidar_array_xy_filt.csv'];
        else
...
%% Store mat files
if Tinfo.filt == 1
    sname = 'szarray_timeseries_allinst_filt';
else
