...
for iT = 1:length(spread)
    ...
    if loadprev == 0
        [press,~] = load_insitu(Tinfo,array);
        IDdef.depth = Tinfo.tide;
        z = press.eta;
        XL       = press.xyz(1,:);
        YL       = press.xyz(2,:);
        ZL       = mean(press.eta,1);%press.xyz(3,:);
        
        %% STEP 4: Select Points
        if complags == 0
            % pick sensors:
            if geom == 1
                sensorpick = [6 11 7 12 4 9 1]; %  BEST SO FAR
            elseif geom == 2
            ...
            for ip = 1:length(sensorpick)
                sensorname = ['p',num2str(sensorpick(ip),'%02.f')];
                %     ix(ip) = ismember(sensorname,wg.loc);
                ix(ip)  = find(strcmp(sensorname,press.loc));
            end
            % ix = [2 4 7 13 10 11];
        end
        ...
        sname = ['E:\data\processed\conditions\','dirspec_cond_comp\pressuregauge_meas_geom',num2str(geom),'.mat'];
    ...
    if smoothz == 0
        Spec    = dat2dspec([Time' Z],[pos types,bfs],H,insHz*(256),361,dirmethod);
    elseif smoothz == 1
        Spec    = dat2dspec([Time' Z],[pos types,bfs],H,(1/tpass)*(256),361,dirmethod);
    end
...
if loadprev ==  0
    psname = ['E:\data\processed\conditions\','dirspec_cond_comp\dirspec_pressgauge_Hs',num2str(100*Tinfo.Hs),'m_geom',num2str(geom),'_',dirmethod,'_wafo_dir',num2str(diff(dirs(1:2))),'.mat'];
    eval(['save -v7.3 ',psname,' sensorpick',' spread',' S',' freqs',' dirs',' Sf',' Sd',' dirsprd_fit',' dirsprd_obs',' Sfit',' dirang',' Sauto',' fauto']);
    
    psname = ['E:\data\processed\conditions\','dirspec_cond_comp\pressuregauge_meas_geom',num2str(geom),'.mat'];
    eval(['save -v7.3 ',psname,' sensorpick',' Z1',' Z2',' Z3',' Z4',' Z5',' xd',' yd',' zd']);
elseif loadprev == 1
    psname = ['E:\data\processed\conditions\','dirspec_cond_comp\dirspec_pressgauge_Hs',num2str(100*Tinfo.Hs),'m_geom',num2str(geom),'_',dirmethod,'_wafo_dir',num2str(diff(dirs(1:2))),'.mat'];
    eval(['save -v7.3 ',psname,' sensorpick',' spread',' S',' freqs',' dirs',' Sf',' Sd',' dirsprd_fit',' dirsprd_obs',' Sfit',' dirang']);
end
