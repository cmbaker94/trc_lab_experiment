...
%     [Tinfo] = wc_comp_store(Tinfo);
    ...
    temp = strfind(cond,'h=');
    wat = str2double(cond(temp+2:temp+5));
...
%     pause(1)
    ...
    [~,istart] = nanmin(abs(time_range(1)-t_inst));
    ...
    for i = 1:length(velnames)
        ...
        h = abs(z-0.05); % inner shelf instruments
        disp_cons = (omega^2)*h/g;
        kh = dispersi(disp_cons);
        k = kh./h;
        c = omega/k;
        top = cosh(2*k*(z+h));
        bottom = (sinh(k*h)^2);
        Ustokes = (1/8)*(hs*k)^2*c*(top/bottom);
%         Ue = -(1/8)*(hs*k)^2*c*(sinh(2*k*h)/(2*k*h));
%         UL = Ustokes+Ue;
        ue(:,instnum) = u(:,instnum)+Ustokes;

%         uLF(:,instnum) = pl64ta(ue(:,instnum),mvavgt*100)-pl64ta(ue(:,instnum),(1/0.0031)*100);
        uLF(:,instnum) = pl64ta(ue(:,instnum),mvavgt*100)-pl64ta(ue(:,instnum),(1/0.006)*100);
%          uLF(:,instnum) = pl64ta(ue(:,instnum),mvavgt*100);
    end
...
Tpplt = [1.5 2 2.5 3];
[c,mu] = polyfit(Tpplt,UTp,1);
[U_est,error] = polyval(c,Tpplt,mu);
plot(Tpplt,U_est,'Color',[1 0 0 0.5],'LineWidth',1.5)
% plot(Tpplt,U_est+error)
% plot(Tpplt,U_est-error)
...
[c,mu] = polyfit(sprdplt,Usprd,1);
[U_est,error] = polyval(c,sprdplt,mu);
plot(sprdplt,U_est,'Color',[1 0 0 0.5],'LineWidth',1.5)
% plot(sprdplt,U_est+2*error)
% plot(sprdplt,U_est-2*error)
plot(sprdplt,U_est,'Color',[1 0 0 0.5],'LineWidth',1.5)
