...
%     display(instnum)
    ...
    eval(['ufull(:,instnum) = vel.u',velnames{i}(4:end),'(1:272013);'])
    eval(['vfull(:,instnum) = vel.v',velnames{i}(4:end),'(1:272013);'])
...
sz.u = ufull;
sz.v = vfull;
sz.u_mean = nanmean(u,1);
sz.v_mean = nanmean(v,1);
clear ufull vfull u v
...
%     display(instnum)
    eval(['u(:,instnum) = vel.u',velnames{i}(4:end),'(istart:iend);'])
    eval(['v(:,instnum) = vel.v',velnames{i}(4:end),'(istart:iend);'])
    eval(['ufull(:,instnum) = vel.u',velnames{i}(4:end),';'])
    eval(['vfull(:,instnum) = vel.v',velnames{i}(4:end),';'])
    ulf(:,instnum) = movmean(u(:,instnum),100*mvavgt)-nanmean(u(:,instnum));
    vlf(:,instnum) = movmean(v(:,instnum),100*mvavgt)-nanmean(v(:,instnum));
    uLF2(:,instnum) = pl64ta(ulf(:,instnum),mvavgt*30);
    vLF2(:,instnum) = pl64ta(vlf(:,instnum),mvavgt*30);
...
is.u = ufull;
is.v = vfull;
is.u_mean = nanmean(u,1);
is.v_mean = nanmean(v,1);
...
i = 1000;
    ...
    imagesc(Y(:,1)',X(1,:)',flipud(rot90(bwim)))
    hold on
    scatter(sz.xyz(:,2),sz.xyz(:,1),40,[186, 94, 242]/256,'fill','MarkerEdgeColor','k')
    quiver(sz.xyz(:,2),sz.xyz(:,1),sz.v_mean'*sf,sz.u_mean'*sf,'AutoScale','off','LineWidth',2,'MaxHeadSize',4,'Color',[186, 94, 242]/256);
    scatter(is.xyz(:,2),is.xyz(:,1),40,[186, 94, 242]/256,'fill','MarkerEdgeColor','k')
    quiver(is.xyz(:,2),is.xyz(:,1),is.v_mean'*sf,is.u_mean'*sf,'AutoScale','off','LineWidth',2,'MaxHeadSize',4,'Color',[186, 94, 242]/256);

    %     text(425,950,'(a)','interpreter','latex','fontsize',20,'Color','w');
    text(11.1,21,'$0.1~\mathrm{m/s}$','interpreter','latex','fontsize',16,'Color','w');
    text(11.2,22,'$\langle \vec{u} \rangle$','interpreter','latex','fontsize',25,'Color','w');
    quiver(11.5,20.5,0,-.1*sf,'AutoScale','off','LineWidth',1.2,'MaxHeadSize',0.7,'Color','w');
    quiver(11.5,20.5,.1*sf,0,'AutoScale','off','LineWidth',1.2,'MaxHeadSize',0.7,'Color','w');
...
%     set(h2, 'Position', [0.72 0.85 0.19 0.064]);
    text(ax1,7.9,18.4,['\textbf{Time Step}: ',datestr((imageno(i)/8)/(24*3600),'MM:SS.FFF')],'interpreter','latex','fontsize',15);
     colormap('gray')
    sname = ['innershelfarray_rectimage_vid_',num2str(imageno(i),'%05.f'),'_full'];
