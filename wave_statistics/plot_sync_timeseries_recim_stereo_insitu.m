...
for i = (((1+reg)*8)+8*180):(((1+reg)*8)+8*300)
    count = count+1;
...
%     IM = imread(imagefile);
    ...
    % plot
    ax1 = axes('Position',[0.053 0.395 0.51 0.4]);
    imagesc(ax1,image.Y(:,1),image.X(1,:),imrotate(I,270));
%     imagesc(IM)
    hold on
    scatter([press.xyz(2,9)],[press.xyz(1,9)],80,'MarkerFaceColor',[255, 110, 231]/256,'MarkerEdgeColor','k','LineWidth',1)
    scatter([press.xyz(2,3)],[press.xyz(1,3)],80,'MarkerFaceColor',[77, 175, 250]/256,'MarkerEdgeColor','k','LineWidth',1)
...
%     set(h1,'YTickLabel',[],'XTickLabel',[]);
    ...
    text(-13.2,25.3,['\textbf{Time}: ',num2str(i/8,'%.2f'),' s'],'interpreter','latex','fontsize',18);
...
%     ylabel('$V$ (pix)','interpreter','latex','fontsize',15);
    ...
    ax2 = axes('Position',[0.053 0.06 0.51 0.4]);
    pcolor(ytemp,xtemp,ztemp-Tinfo.tide-0.02)
    hold on
    scatter([press.xyz(2,9)],[press.xyz(1,9)],80,'MarkerFaceColor',[255, 110, 231]/256,'MarkerEdgeColor','k','LineWidth',1)
    scatter([press.xyz(2,3)],[press.xyz(1,3)],80,'MarkerFaceColor',[77, 175, 250]/256,'MarkerEdgeColor','k','LineWidth',1)
...
%     text(ax2,press.xyz(2,3)-0.35,press.xyz(1,3)-0.6,'p11','interpreter','latex','fontsize',15)
    ...
    hc = colorbar('Position', [0.575 0.1 0.02 0.26]);
    caxis([-0.15 0.15])
    axis equal
%     grid on
    box on
    ...
    text(ax2,13.95,26.25,'$\eta$ (m)','interpreter','latex','fontsize',18);
...
%     text(ax2,24.7,14.2,['time: ',num2str(round(imageno(i)/8,3),'%.3f'),' sec'],'interpreter','latex','fontsize',15);    
    ...
    ax3 = axes('Position',[0.68 0.41 0.3 0.28]);
...
%     hold on
    ...
    fill([-0.04 -0.04 0.04 0.04],[-0.3 0.5 0.5 -0.3],[239 242 157]/256,'LineStyle','none','EdgeColor','none')
%     scatter(Ito,Izoff,4,[0.8 0.8 0.8],'fill')
    scatter(Ito,Ifzoff,4,'k','fill')
    scatter(Cto,Czoff,8,'r','fill')
    scatter(Lto,Lzoff,8,'b','fill')
...
%     grid on
    ...
    ylabel('$\eta$ (m)','interpreter','latex','fontsize',18);
...
%     text(ax3,-3.9, 0.43,['(c) p06, $x=$',num2str(round(press.xyz(1,9),1)),' m'],'interpreter','latex','fontsize',15);
    ...
    ax4 = axes('Position',[0.68 0.1 0.3 0.28]);
    plot([min(Cto) max(Cto)],[0 0],'LineWidth',1,'LineStyle','-','Color',[.8 .8 .8])
    hold on
    fill([-0.04 -0.04 0.04 0.04],[-0.2 0.2 0.2 -0.2],[239 242 157]/256,'LineStyle','none','EdgeColor','none')
...
%     grid on
    ...
    xlabel('Time Offset (s)','interpreter','latex','fontsize',18);
    ylabel('$\eta$ (m)','interpreter','latex','fontsize',18);
