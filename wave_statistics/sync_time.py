...
procpath = 'E:/data/processed/conditions/'
font = {'family' : 'normal',
        'weight' : 'normal',
        'size'   : 18}
...
figfolder = 'E:/figures/conditions/' + Tinfo['comp'] + '/' + Tinfo['trialstart']+ '/time_' + Tinfo['timerange'] + '/' + date + '/'
...
# %% Try with other sensors

# drs_pc = np.zeros(len(di.columns[:-1]))
# drs_pl = np.zeros(len(di.columns[:-1]))
# drs_cp = np.zeros(len(di.columns[:-1]))
# drs_cl = np.zeros(len(di.columns[:-1]))
# drs_lp = np.zeros(len(di.columns[:-1]))
# drs_lc = np.zeros(len(di.columns[:-1]))

# count = 0
# for sens in di.columns[:-1]:
#     print(sens)
    
#     tpes = dpes[sens].reset_index(drop = True) # same result for pressure and depth-attenuation corrected
#     tls = dls['l'+sens[1:]].reset_index(drop = True) # referenced to lidar since that is started first
#     tc = dc['c'+sens[1:]].reset_index(drop = True)
#     drs_pc[count] = np.argmax(np.absolute([crosscorr(tpes,tc, lag) for lag in range(0,seconds*fps)]))
#     drs_pl[count] = np.argmax(np.absolute([crosscorr(tpes,tls, lag) for lag in range(0,seconds*fps)]))
#     drs_cp[count] = np.argmax(np.absolute([crosscorr(tc,tpes, lag) for lag in range(0,seconds*fps)]))
#     drs_cl[count] = np.argmax(np.absolute([crosscorr(tc,tls, lag) for lag in range(0,seconds*fps)]))

#     drs_lp[count] = np.argmax(np.absolute([crosscorr(tls,tpes, lag) for lag in range(0,seconds*fps)]))
#     drs_lc[count] = np.argmax(np.absolute([crosscorr(tls,tc, lag) for lag in range(0,seconds*fps)]))
#     count = count+1
...
ax.plot(rs_cl)
ax.axvline(np.argmax(rs_cl),color='r',linestyle='--',label='Peak synchrony')
plt.legend()
plt.show()
...
for inst in press_gauges:
    for tp in range(0,6):
            ...
            if inst == '11':
                ...
                fig, axs = plt.subplots(1, figsize=(14,4), facecolor='white') # was 20x6
            # axs.plot(dpe['date'], dpe['p' + inst], c='y', lw=0.4, label='hyd press')
            # axs.scatter(dpe['date'], dpe['p' + inst], s = 0.3, marker='.', c='y')
            axs.plot(di['date'], di['p' + inst], c='k', lw=0.4, label='in situ')
            axs.scatter(di['date'], di['p' + inst], s = 0.3, marker='.', c='k')
            axs.plot(dc['date'], dc['c' + inst], c='r', lw=0.7, label='camera')
            axs.scatter(dc['date'], dc['c' + inst], marker='.',s = 4,  c='r')
            # axs[2].plot(dl['date'], dl['l12'], c?='k', lw=0.3, label='lidar')
            ...
            # axs.get_xaxis().set_ticklabels([])
            axs.set_xlabel('time (hh:mm:ss)')
            axs.set_xlim((dl['date'].iloc[600*trange[0]],dl['date'].iloc[600*trange[1]])) # was to 4000
            if inst == '11':
                axs.set_ylim(-0.2,0.2)
            elif inst == '06':
                axs.set_ylim(-0.2,0.55)
