...
def timeseries_Kp_correction(press,fs,maxfac,offset):
    ...
    See[np.abs(fftfreq) > maxfac] = 0# not including 0 filt now
    # nyq = 0.5 * fs
    # lowcut = maxfac / nyq
    # order = 2
...
if Tinfo['spread'] == 0:
    ...
    Tinfo['timerange'] = '2241-2250'
    Tinfo['trialstart'] = '08-30-2018-2226UTC'
...
rs_pc = [crosscorr(tpes,tc, lag) for lag in range(0,seconds*fps)]
rs_pl = [crosscorr(tpes,tls, lag) for lag in range(0,seconds*fps)]
rs_cp = [crosscorr(tc,tpes, lag) for lag in range(0,seconds*fps)]
rs_cl = [crosscorr(tc,tls, lag) for lag in range(0,seconds*fps)]
...
# pressure
dif = di.interpolate(method='linear', axis=0)
# dif = pd.DataFrame()
fs = 100       # sample rate, Hz
for (columnName, columnData) in dif.iloc[:, :-1].iteritems():
    presstemp = columnData.values
    dif[columnName] = butter_lowpass_filter(presstemp, cutoff, fs, order)
    
# camera
dcf = dc.interpolate(method='linear', axis=0)
# dcf = pd.DataFrame()
fs = 8       # sample rate, Hz
for (columnName, columnData) in dcf.iloc[:, :-1].iteritems():
    camtemp = columnData.values
    dcf[columnName] = butter_lowpass_filter(camtemp, cutoff, fs, order)
   
# lidar
dlf = dl.interpolate(method='linear', axis=0)
# dlf = pd.DataFrame()
fs = 10       # sample rate, Hz
for (columnName, columnData) in dlf.iloc[:, :-1].iteritems():
    lidartemp = columnData.values
    dlf[columnName] = butter_lowpass_filter(lidartemp, cutoff, fs, order)
...
# data = presstemp
# t = np.linspace(0, len(presstemp)/100, len(presstemp), endpoint=False)
...
for inst in press_gauges:
    ...
    for tp in range(0,9):
        trange = [tp, tp+1]
        if inst == '11':
            fig, axs = plt.subplots(1, figsize=(20,4), facecolor='white') # was 20x4
        elif inst == '06':
            fig, axs = plt.subplots(1, figsize=(20,6), facecolor='white') # was 20x6
        # axs.plot(dpe['date'], dpe['p' + inst], c='y', lw=0.4, label='hyd press')
        # axs.scatter(dpe['date'], dpe['p' + inst], s = 0.3, marker='.', c='y')
        axs.plot(di['date'], dif['p' + inst], c='k', lw=0.4, label='dac press')
        axs.scatter(di['date'], dif['p' + inst], s = 0.3, marker='.', c='k')
        axs.plot(dc['date'], dcf['c' + inst], c='r', lw=0.7, label='camera')
        axs.scatter(dc['date'], dcf['c' + inst], marker='.',s = 4,  c='r')
        # axs[2].plot(dl['date'], dl['l12'], c?='k', lw=0.3, label='lidar')
        axs.plot(dl['date'], dlf['l' + inst], c='b', lw=0.7, label='lidar')
        axs.scatter(dl['date'], dlf['l' + inst], marker='.',s = 4,  c='b')
        ...
        axs.set_xlim((dpe['date'].iloc[6000*trange[0]],dpe['date'].iloc[6000*trange[1]]))
        if inst == '11':
        ...
        plt.savefig(figfolder + Tinfo['comp'] + '_timeseries_p' + inst + '_z' + str(trange[0]) + '_filtered_long.png', bbox_inches='tight')
        plt.show()
        
# %%        
count = 0
fig, axs = plt.subplots(3,2, figsize=(20,12), facecolor='white')
for ax in fig.get_axes():
    ax.plot(di['date'], dif['p' + inst], c='k', lw=0.4, label='dac press')
    ax.scatter(di['date'], dif['p' + inst], s = 0.3, marker='.', c='k')
    ax.plot(dc['date'], dcf['c' + inst], c='r', lw=0.7, label='camera')
    ax.scatter(dc['date'], dcf['c' + inst], marker='.',s = 4,  c='r')
    ax.plot(dl['date'], dlf['l' + inst], c='b', lw=0.7, label='lidar')
    ax.scatter(dl['date'], dlf['l' + inst], marker='.',s = 4,  c='b')
    ax.hlines(0, dl['date'].min(), dl['date'].max(), colors='grey', linestyles='-',alpha = 0.25)
    ax.get_xaxis().set_ticklabels([])
    ax.set_xlabel('time (hh:mm:ss)')
    # ax.set_xlim((dpe['date'].iloc[6000*trange[0]],dpe['date'].iloc[6000*trange[1]])) # was to 4000
    ax.set_xlim((dpe['date'].iloc[6000*trange[count]],dpe['date'].iloc[6000*trange[count+1]]))
    if inst == '11':
        ax.set_ylim(-0.2,0.2)
    elif inst == '06':
