...
Tinfo.tide = 1.07;
Tinfo.filt = 1;
hyd= 0; % zero means depth attenuation correction, 1 means pressure in units (N/m/m)
...
EP.dres       = 360;
EP.method     = 'EMEP';%'EMLM' 'IMLM' 'EMEP' 'BDM'
if hyd == 0
...
if Tinfo.filt == 1
    if hyd == 0
        specsave = [Tinfo.savefolder,'dirspec_press_sz_rand_',num2str(instnum),'inst_',num2str(iternum),'iter'];
    elseif hyd == 1
...
Sname1 = [Tinfo.figfolder,'dirspec_pres_inst',num2str(instnum),'_iter',num2str(iternum),'_',EP.method];
print(Sname1,'-dpng')
