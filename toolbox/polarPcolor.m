...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function createSpokes(thetaMin,thetaMax,Ncircles,circlesPos,Nspokes)
        ...
        spokeMesh = [0    45    90   135   180   225   270   315   359];
            ...
            if and(thetaMin==0,thetaMax == 360),
                if spokeMesh(kk)<360,
                    ...
                    text(1.05.*contourD(end).*cost(kk),...
                        ...
                        'horiz', 'center', 'vert', 'middle','fontsize',14);
                end
            elseif spokeMesh(kk)<359
                text(1.11.*contourD(end).*cost(kk),...
                    1.11.*contourD(end).*sint(kk),...
                    [num2str(spokeMesh(kk),3),char(176)],...
                    'horiz', 'center', 'vert', 'middle','fontsize',14);
            end
    ...
    function createCircles(rMin,rMax,thetaMin,thetaMax,Ncircles,circlePos,Nspokes)
        ...
        for kk=1:Ncircles
            ...
            if isempty(RtickLabel),
                rtick = num2str(round(tickMesh(kk),1),2);
            else
                rtick = RtickLabel(kk);
            end
            if abs(round(position)) ==90,
                ...
                % radial graduations
               t = text(0.95*contourD(kk).*cosd(position),...
                    0.95*(contourD(kk)).*sind(position),...
                    rtick,'verticalalignment','BaseLine',...
                    'horizontalAlignment', 'right',...
                    'handlevisibility','off','parent',cax,...
                    'color','w','fontsize',14);
